...
pdf(file = paste0(dir_out, "Fig4_A_UMAP_all_cell_by_tissue_", today, ".pdf"), width = 7, height = 7)
p + theme(legend.position = "none")
grid.arrange(p_legend)
dev.off()

cairo_pdf(file = paste0(dir_out, "Fig4_A_UMAP_all_cell_by_tissue_", today, ".pdf"), width = 7, height = 7)
p + theme(legend.position = "none")
# grid.arrange(p_legend)
dev.off()

# svg(file = paste0(dir_out, "Fig4_A_UMAP_all_cell_by_tissue_", today, ".svg"), width = 7, height = 7)
# p + theme(legend.position = "none")
# # grid.arrange(p_legend)
# dev.off()

# library(tools)
# compactPDF(dir_out, gs_quality = "printer")
```
...
p <- ggplot(plt_df, aes(x = UMAP_1, y = UMAP_2, color = treatment)) + 
  ...
  theme(legend.title = element_text(size = 15),
        ...
        axis.text = element_text(size = 12))
p_legend <- get_legend(p)
pdf(file = paste0(dir_out, "Fig4_B_UMAP_all_cell_by_treatment_", today, ".pdf"), width = 7, height = 7)
p + theme(legend.position = "none")
grid.arrange(p_legend)
dev.off()
```
...
p <- ggplot(plt_df, aes(x = UMAP_1, y = UMAP_2, color = treatment)) + 
  ...
  theme(legend.title = element_text(size = 15),
        ...
        axis.text = element_text(size = 12))
p_legend <- get_legend(p)
pdf(file = paste0(dir_out, "Fig4_C_UMAP_SPL_by_treatment_", today, ".pdf"), width = 7, height = 7)
p + theme(legend.position = "none")
grid.arrange(p_legend)
dev.off()
```
...
pdf(file = paste0(dir_out, "Fig4_D_MAplot_", today, ".pdf"), width = 5, height = 5)
p
dev.off()
...
## Figure 4E: Corrplot for TCR sharing across tissues
```{r}
date_analysis <- "2021-01-21"#"2020-12-10"
...
pdf(file = paste0(dir_out, "Fig4_E_corrplot_clonal_similarity_", today, ".pdf"), width = 7, height = 7)
...
## Figure 4F: Heatmap for TCR sharing across tissues
```{r}
library(pheatmap)
...
saveRDS(p, file = paste0(dir_out, "Fig4_F_heatmap_clone_size_row_percentage_", today, ".rds"))

pdf(file = paste0(dir_out, "Fig4_F_heatmap_clone_size_row_percentage_", today, ".pdf"), width = 4, height = 6)
