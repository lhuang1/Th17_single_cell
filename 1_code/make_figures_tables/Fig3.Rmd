...
write.table(meta, paste0(dir_proj, "2_pipeline/clustering/EAE_GFPall_intra/", 
                          clustering_date, "/FILES/CNS/meta_data_switched.csv"), sep = ",")
...
pdf(file = paste0(dir_out, "Fig3_B_UMAP_by_cluster_", today, ".pdf"), width = 7, height = 7)
...
p
pdf(paste0(dir_out, "Fig3_C_cluster_size_pie_", today, ".pdf"), width = 5, height = 5)
p
dev.off()
```
...
# CD8-like: C4 -> C3
# Proinflammatory: C2 -> C4
cluster_new <- as.character(markers$cluster) %>% as.numeric()
cluster_new[markers$cluster == 1] <- 0
cluster_new[markers$cluster == 0] <- 1
cluster_new[markers$cluster == 3] <- 2
cluster_new[markers$cluster == 4] <- 3
cluster_new[markers$cluster == 2] <- 4
markers$cluster <- cluster_new
saveRDS(markers, paste0(dir_proj, "2_pipeline/clustering/EAE_GFPall_intra/", clustering_date, "/FILES/CNS/so_markers_switched.rds"))
...
pdf(file = paste0(dir_out, "Fig3_D_heatmap_", today, ".pdf"), width = 7, height = 7)
grid.arrange(p$gtable)
dev.off()

write.csv(deg_unique, file = paste0(dir_out, "DEG_cluster_", today, ".csv"))
...
# so_cap[['integrated']]@data <- apply(so_cap[['integrated']]@data, 1, function(x) {pmax(pmin(x, quantile(x, 0.995)), quantile(x, 0.005))}) %>% t
...
plist <- lapply(glist, function(g){
  ...
  if (g %in% c("Tmsb10", "Samhd1", "Tnfrsf9", "Nkg7")) { ## only show for first column
    p <- p + theme(axis.title.y = element_text(color = "black"))
  } else {
    p <- p + theme(axis.title.y = element_text(color = "white"))
  }
  
  if (g %in% c("Tnfrsf9", "Ifng", "Csf2", "Il17a-GFP", "Areg")) { ## only show for last row
    p <- p + theme(axis.title.x = element_text(color = "black"),
...
pdf(paste0(dir_out, "Fig3_E_violon_20pct_", today, ".pdf"), height = 12, width = 16)
...
## switch sheet order so we have quiesent -> migratory -> prolif -> Cd8 -> proinflammatory
# Quiescent: C1 -> C0
# Migratory: C0 -> C1
# Proliferating: C3 -> C2
# CD8-like: C4 -> C3
...
sheet_vec_switched <- c("C_1", "C_0", "C_4", "C_2", "C_3")
sheet_vec_reorderd <- c("C_1", "C_0", "C_3", "C_4", "C_2")

## recompute FDR
gsea <- readRDS(paste0(dir_proj, "2_pipeline/enrichment/CNS_clusters/", enrichment_date, "/GSEA_outputs.rds"))
for (x in sheet_vec){
  fdr <- p.adjust(c(gsea$go_bp[[x]]$pval, gsea$kegg[[x]]$pval), method = "fdr")
  gsea$go_bp[[x]]$FDR <- fdr[1:nrow(gsea$go_bp[[x]])]
  gsea$kegg[[x]]$FDR <- fdr[(nrow(gsea$go_bp[[x]]) + 1):length(fdr)]
}
## fix cluster names
gsea_go_bp_all <- lapply(seq_along(sheet_vec), function(i){
  y <- gsea$go_bp[[sheet_vec[i]]]
  y$Cluster <- sub("_", "", sheet_vec_switched[i])
  y}) %>% Reduce(rbind, .)
...
d <- lapply(sheet_vec_reorderd, function(x){
...
pathway_name_vec <- lapply(sheet_vec_reorderd, function(x){
  tmp <- readWorkbook(xlsxFile = xlsx_file, sheet = x)
  tmp$Name.for.plot
}) %>% unlist
...
p_pval <- d %>% 
  ...
  # geom_point(aes(x = Cluster, y = In.figure, size = -log10(pval), fill = NES), shape = 21) +
  ...
  theme(axis.text.x = element_text(color = "black"),
        # axis.text.y = element_text(hjust = -0.01, vjust = 0.5),
        legend.title = element_text(size = 8),
        legend.box = "vertical", legend.position="right", legend.key.size = unit(.13, "in")) 
p_pval


pdf(paste0(dir_out, "Fig3_F_enrichment_", today, ".pdf"), height = 4, width = 4.5)
p_pval
dev.off()
```
...
pdf(paste0(dir_out, "Fig3_G_piechart_", today, ".pdf"))
...
# [1] 0.07289574 0.03889440 0.12990328 0.03001709 0.02315242 (before switching cluster labels)
# [1] 0.03889440 0.07289574 0.03001709 0.02315242 0.12990328 (after)
```
