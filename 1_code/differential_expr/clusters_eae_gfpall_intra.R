...
dir_out <- paste0("2_pipeline/differential_expression/Cluster_EAE_GFPall_intra/", today, "/")
...
meta <- read.table(paste0("2_pipeline/clustering/EAE_GFPall_intra/", clustering_date, "/FILES/", tissue, "/meta_data.csv"), sep = ",", row.names = 1, header = T)
...
# prefilter genes
gene_prefilter <- 0.001 # a gene should be detected by this proportion in at least one of the tissues
