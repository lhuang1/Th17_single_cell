...
sigs_raw_gaublomme <- sigs_raw_gaublomme %>% mutate(sig_name = paste(V1, V2, sep = "-")) %>%
  group_by(sig_name) %>% summarise(genes = paste(V3, collapse = ","))
...
# Interferon signatures from literature
...
# pathogenic th17 from Lee, 2012
sigs_patho17 <- list("Pathogenic_Th17" = c("Cxcl3", "Il22", "Il3", "Ccl4", "Gzmb", "Lrmp", "Ccl5", "Casp1", "Csf2", "Ccl3", "Tbx21", "Icos", "Il7r", "Stat4", "Lgals3", "Lag3"))
